...
$(function(){
	...
	$("[name='easyTip']").each(function() {
		//如果不是必填项，则去除正确验证提示
		$(this).addClass("onShow");//zhongying(2014-01-add)-start
		
		var regStr=$(this).prev("div").children("input").attr("reg");//^$|
		if(regStr!= undefined && regStr.indexOf("^$|")==0){
			$(this).removeClass();//zhongying(2014-01-add)-start
		}
	...
	$("[reg]").click(function() {
		...
		$(this).parent().nextAll("[name='easyTip']").eq(0).addClass("onFocus");
	});
...
function validate(obj){
	...
	if(!reg.test(objValue)){
		tipObj.addClass("onError");//zhongying(2014-01-add)-start
...
function change_error_style(obj,action_type){
	...
	if(action_type == "add"){
		...
		$(obj).parent().next("span[name='easyTip']").addClass("onError");//zhongying(2014-02-add)-start
		...
		var regStr=$(obj).attr("reg");//^$|
		if(regStr!= undefined && regStr.indexOf("^$|")!=0){
			$(obj).parent().next("span[name='easyTip']").addClass("onCorrect");//zhongying(2014-01-add)-start
		}
