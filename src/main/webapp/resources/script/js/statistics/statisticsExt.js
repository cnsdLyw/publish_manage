...
//趋势分析
function statTrend(bookId,trendType){
	$("#statText").html('趋势分析');
	$("#statFlag").val(1);
	...
	$.ajax( {  
        type : "post",
        async: false,
        url : "../statisticsExt/trend/",
        data:{bookId:bookId,year:$("#chooseYear").val(),trendType:trendType},
        dataType:"json",  
...
//特征分析
function statFeatures(featFlag,bookId){
	$("#statText").html('特征分析');
	$("#statFlag").val(2);
	$("#main").css({height:"450px"});
	var maxValue;
	...
	$.ajax( {  
        type : "post",
        async: false,
        url : "../statisticsExt/features",
        data:{featFlag:featFlag,bookId:bookId,year:$("#chooseYear").val()},
        dataType:"json",  
...
function statRegion(bookId){
	...
	$.ajax( {  
        type : "post",
        async: false,
        url : "../statisticsExt/region",
        data:{bookId:bookId,year:$("#chooseYear").val()},
        dataType:"json",  
        success : function(dataStr) {
       		dataDG = eval(dataStr.list1);
			dataXL = eval(dataStr.list2);
			dataKC = eval(dataStr.list3);
			dataTH = eval(dataStr.list4);
			dataJY = eval(dataStr.list5);
            flag = dataStr.flag;
        }  
    });  
	 // 使用
    ...
    function (ec) {
        ...
        var option = {
	        ...
	        if(dataDG!=null && dataDG!="undified"){
	         ...
	         if(dataTH!=null && dataTH!="undified"){
		         var itemTH={
	 		        	name:'退货',
	 		        	type:'map',
	 		        	data:dataTH,
