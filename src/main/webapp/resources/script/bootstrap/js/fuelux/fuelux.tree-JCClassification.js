			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder') {
						...
						$entity.find('.tree-branch-name > .tree-label').attr('id',value.code+"item");
						//ACE
						...
						if('additionalParameters' in value
							&& 'itemSelected' in value.additionalParameters 
								&& value.additionalParameters['itemSelected'] == true) {
						...
						$entity.find('.tree-item-name > .tree-label').attr('id',value.code+"item");
						$entity.data(value);
						//alert($("#"+value.id+"item"));
						...
						new BootstrapMenu("#"+value.code+"item", {
						  actions: [{
								name: '添加分类',
								...
								name: '删除分类',
								iconClass: 'fa-trash red',
								onClick: function() {
									window.parent.deleteClass(value.code);
								}
							  }]
						});
