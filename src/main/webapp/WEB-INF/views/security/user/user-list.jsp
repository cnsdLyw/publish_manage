				  ...
				  <div class="modal-dialog">
					...
					<form action="${ctx }/user/list" method="post" id="queryForm">
						...
						<div class="page-header">
							<div>
								<security:authorize ifAnyGranted="ROLER_SYS_USER_NEW">
									<button class="btn btn-sm btn-primary " type="button"  onclick="javascript:location.href='${ctx}/user/addUser'" id="demo1Box">
										新建</button>&nbsp;&nbsp;&nbsp;
								</security:authorize>
								<security:authorize ifAnyGranted="ROLER_SYS_USER_DELETE">		
									<button class="btn btn-sm btn-primary " type="button" onclick="javascript:deleteAll();">删除</button>&nbsp;&nbsp;&nbsp;
								</security:authorize>
								<security:authorize ifAnyGranted="ROLER_SYS_USER_REFRESH">
								...
								<div class="col-md-4 col-sm-2 pull-right">
									...
									<input type="text" class="col-md-5 col-sm-2 pull-right" id="keyWord" name="keyWord"  value="${keyWord }" placeholder="登录名|姓名" data-rel="tooltip" title="登录名|姓名">
								</div><!-- 表单检索row  end-->
							</div>
						</div><!-- 操作按钮end -->
										...
										<table id="dynamic-table"
											...
											<thead>
												<tr user="row">
													...
													<th class="sorting" id="loginName" tabindex="0" onclick="javascript:sort(this,'queryForm','loginName','asc');">登录名</th>
													<th class="sorting" id="name" tabindex="0" onclick="javascript:sort(this,'queryForm','name','asc');">姓名</th>
													<th class="sorting" id="email" tabindex="0" onclick="javascript:sort(this,'queryForm','email','asc');">邮箱</th>
													<th>操作</th>
												</tr>
											</thead>
											<tbody>
												 <c:forEach  items="${pageContent.content}" var="user">
													<tr user="row">
														<td class="center"><label class="pos-rel"> 
															<input name="ids" type="checkbox" class="ace" value="${user.id}"> <span class="lbl"></span>
														</label>
														</td>
														...
														<td>
															...
															<security:authorize ifAnyGranted="ROLER_SYS_USER_EDIT">	
																<a class="green" href="${ctx }/user/editUser/${user.id}.html" title="点击修改"> <i
																		class="ace-icon fa fa-pencil bigger-130"></i>
																</a> &nbsp;&nbsp;&nbsp;
															</security:authorize>
															<security:authorize ifAnyGranted="ROLER_SYS_USER_UPDATEPASSWORD">	
																<a class="green" href="${ctx }/user/editPassword/${user.id}.html" title="点击修改登录密码"> <i
																		class="ace-icon fa fa-check-square-o bigger-130"></i>
																</a> &nbsp;&nbsp;&nbsp;
															</security:authorize>															
															<security:authorize ifAnyGranted="ROLER_SYS_USER_DELETE">	
																<a class="red" href="javascript:void(0);" title="点击删除"  onclick="deleteUser(${user.id})"> <i
