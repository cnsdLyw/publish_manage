		...
		if(ids){
		    ...
		    bootbox.confirm("确认删除，删除后不可恢复!", function (result) {  
                if(result) {//确认删除  
                	...
                	document.location.href="${ctx}/sysConfirure/delete/"+ids+".html?typeid=${typeid}";
                } 
         });  
		}else{
			...
			<div class="main-content-inner">
				...
				<div class="modal fade" id="templatemo_modal" tabindex="-1" user="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				  <div class="modal-dialog">
					...
					<div class="col-md-12">
					<form action="${ctx }/sysConfirure/list?typeid=${typeid}" method="post" id="queryForm">	
					 <input type="hidden" id="pageNo" name="pageNo" />
						...
						<div class="page-header">
							...
							<c:if test="${parentId==0}">
								...
								<button class="btn btn-sm btn-primary " type="button"  onclick="javascript:location.href='${ctx}/sysConfirure/addSysConfig?typeid=${typeid}'">
									新建</button>&nbsp;&nbsp;&nbsp;
								<!-- 
								...
								<div class="col-md-4 col-2 pull-right">
									...
									<input type="text" class="col-md-5 col-2 pull-right" id="keyWord" name="keyWord"  value="${keyWord }" placeholder="配置名称|配置编码" data-rel="tooltip" title="配置名称|配置编码">
								</div><!-- 表单检索row  end-->
							</div>
						</div><!-- 操作按钮end -->
										...
										<table id="dynamic-table"
											...
											<thead>
												<tr user="row">
													...
													<th tabindex="0">配置名称</th>
													<th tabindex="0">配置编码</th>
													<th tabindex="0">配置值</th>
													<th>操作</th>
												</tr>
											</thead>
											<tbody>
											<c:forEach items="${pageContent.content}" var="sysConfig">
													<tr user="row" class="odd">
														<td class="center"><label class="pos-rel"> 
															<input name="ids" type="checkbox" class="ace" value="${sysConfig.id}"> <span class="lbl"></span>
														</label>
														</td>
														<td>${sysConfig.configureName}</td>
														<td>${sysConfig.configureCode}</td>
														<td>${sysConfig.configureValue}</td>
														<td>
															<a class="red" href="#" class="test" onclick="getSysConfigure(${sysConfig.id})" data-toggle="modal" data-target="#templatemo_modal" title="查看详情">  <i
																	class="ace-icon fa fa-book bigger-130"></i>
															</a> &nbsp;&nbsp;&nbsp; 
															<a class="red" href="${ctx }/sysConfirure/editSysConfig/${sysConfig.id}?typeid=${typeid}" title="点击修改"> <i class="ace-icon fa fa-pencil bigger-130"></i>
															</a> &nbsp;&nbsp;&nbsp;
														</td>
													</tr>
													</c:forEach>
